 ...
 *
 * Номер показывается полностью и ведёт на звонок — форму перед показом номера
 * не просим.
 */
 ...
 * проекта, и в интерфейсе показываются все его номера. Набор хранится отдельно
...
export const PROJECT_PHONES = {
  'white-hill': {
    ...
    whatsapp: '77021118070',
    numbers: [
      { full: '+7 702 111-80-70', href: 'tel:+77021118070' },
      { full: '+7 778 111-80-70', href: 'tel:+77781118070' },
    ],
  },
};
...
/**
 * @param {string} city
 * @returns {{ full: string, href: string, whatsapp: string }}
 */
 ...
 * @param {string} slug
 * @returns {{ full: string, href: string, whatsapp: string, numbers: Array<{ full: string, href: string }> }|null}
 */
 ...
 * несколько, поэтому обходить их удобнее одним списком, не разбирая, откуда
 * пришёл номер.
 * @param {{ full: string, href: string, numbers?: Array<{ full: string, href: string }> }|null} phone
 * @returns {Array<{ full: string, href: string }>}
 */
export function phoneNumbers(phone) {
  if (!phone) return [];
  return phone.numbers ?? [{ full: phone.full, href: phone.href }];
}
 ...
 * поэтому номер сопоставляется с записью именно по ней.
 * @param {string} href
 * @returns {{ full: string, href: string, whatsapp: string }|null}
 */
